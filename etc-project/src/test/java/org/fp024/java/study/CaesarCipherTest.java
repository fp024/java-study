...
class CaesarCipherTest {
  private static final String DECODE_DICTIONARY = createDictionary(1);
        ...
        .forEach(
            key ->
                jsonObject.addProperty(
                    String.format("%c", key), String.format("%c", getCode(key, shift))));
    jsonObject.addProperty(" ", " ");
  ...
  private static String processText(String text, String dictionary) {
    ...
    return text.chars()
        .mapToObj(key -> jsonObject.get(String.format("%c", key)).getAsString())
